...
// pixel format
//    Mono16 preserves the sensor's native bit depth instead of collapsing it
//    into an 8-bit RGB triplication (BGR8/RGB8 on a mono sensor just copies
//    the single channel into R, G, and B, tripling file size for zero extra
//    information and throwing away the upper bits of dynamic range).
...
#define FILE_NAME "Images/Cpp_Save_Mono16/image"
#define FILE_EXT ".png"
...
int main()
{
	...
	try
	{
		...
		// set pixel format to Mono16 so acquisition captures the sensor's full
		// native bit depth instead of the driver's default (often Mono8 or an
		// RGB conversion), before the stream starts
		Arena::SetNodeValue<GenICam::gcstring>(pDevice->GetNodeMap(), "PixelFormat", "Mono16");
